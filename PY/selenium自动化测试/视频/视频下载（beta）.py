...
class DownloadThread(QThread):
    ...
    def run(self):
        try:
            download_m3u8(self.url, self.headers, self.selected_episodes)  # 调用下载函数下载视频
...
class MainWindow(QWidget):
    ...
    def start_download(self):
        ...
        self.thread = DownloadThread(url, headers, selected_episodes)  # 创建下载线程
        self.thread.progress_update.connect(self.update_progress)  # 连接信号到更新下载进度的方法
        self.thread.message_box.connect(self.show_message)  # 连接信号到显示消息框的方法
        self.thread.start()  # 启动下载线程

        self.download_button.setEnabled(False)  # 禁用开始下载按钮，避免重复点击

    def update_progress(self, current, total):
        pass  # 占位符，用于更新下载进度的逻辑

    def show_message(self, message):
        QMessageBox.information(self, '提示', message)  # 显示信息框，显示提示信息
        self.download_button.setEnabled(True)  # 启用开始下载按钮
...
def get_episode_numbers(url, headers):
    ...
    #提取影视名称/html/body/div[1]/div[3]/div[2]/div[2]/div[2]/div[1]/strong
    video_name=tree.xpath('/html/body/div[1]/div[3]/div[2]/div[2]/div[2]/div[1]/strong/text()')[0]
    # print(video_name)

    urls = tree.xpath('/html/body/div[1]/div[3]/div[2]/div[4]/div[2]/div[1]/a/@href')  # 使用 XPath 获取所有视频集的链接
    episode_numbers = list(range(1, len(urls) + 1))  # 根据链接数量生成视频集数列表，从 1 开始

    return episode_numbers  # 返回视频集数列表

def download_video_segment(segment_url, headers, segment_path, max_retries=30):
    for attempt in range(max_retries):
        try:
            res = requests.get(segment_url, headers=headers, timeout=120)  # 发送 HTTP GET 请求下载视频片段
            if res.status_code == 200:
                with open(segment_path, "wb") as f:
                    f.write(res.content)  # 将下载的视频片段内容写入文件
                return True  # 下载成功，返回 True
            else:
                print(f"\n下载失败，HTTP 状态码: {res.status_code}，重试次数：{attempt + 1}")  # 打印下载失败信息
        except requests.exceptions.RequestException as e:
            print(f"\n下载失败正在重新下载，重试次数：{attempt + 1}")  # 打印异常信息，重试下载

    return False  # 超过最大重试次数仍然下载失败，返回 False
...
def download_m3u8_videos(m3u8_url, headers, episode_number):
    ...
    if response.status_code != 200:
        print(f"无法获取 m3u8 文件: {m3u8_url}")  # 打印无法获取 m3u8 文件的错误信息
        return
    ...
    matches = re.findall(r'\b[A-Za-z0-9]+\.bin\?sign=[a-f0-9]{32}&timestamp=\d+\b', response.text)  # 使用正则表达式匹配所有视频片段的 URL

    if not matches:
        print(f"未找到视频片段: {m3u8_url}")  # 打印未找到视频片段的错误信息
        return

    episode_folder = f"第{episode_number}集"  # 设置视频集的文件夹名称
    ...
    with TPE(max_workers=32) as executor:  # 使用线程池执行器，并发下载视频片段
        ...
        progress = tqdm(total=len(matches), desc=f"下载第{episode_number}集进度：", unit="file")  # 使用 tqdm 显示下载进度条
        for i, match in enumerate(matches):
            segment_url = base_url + match  # 拼接完整的视频片段 URL
            segment_path = os.path.join(episode_folder, f"video_{i}.mp4")  # 设置视频片段的保存路径
...
def download_episode(episode_url, headers, episode_number):
    ...
    if m3u8_matches:
        ...
        download_m3u8_videos(m3u8_url, headers, episode_number)  # 调用下载 m3u8 视频的函数
    else:
...
def download_m3u8(url, headers, episode_numbers):
    ...
    with TPE(max_workers=len(episode_numbers)) as executor:  # 使用线程池执行器，并发下载剧集
        ...
        for episode_number, episode_url in zip(episode_numbers, selected_urls):
            future = executor.submit(download_episode, episode_url, headers, episode_number)  # 提交下载任务到线程池
            futures.append(future)  # 将 future 对象添加到列表中，用于获取下载结果
...
if __name__ == '__main__':
    app = QApplication([])  # 创建 Qt 应用程序
    window = MainWindow()  # 创建主窗口
    app.exec_()  # 运行 Qt 应用程序
