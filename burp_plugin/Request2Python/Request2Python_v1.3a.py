...
class BurpExtender(IBurpExtender, IContextMenuFactory, IMessageEditorTabFactory):
    ...
    def createMenuItems(self, invocation):
        ...
        # TODO: 新增原始请求导出菜单项
        raw_export_item = JMenuItem(u"导出原始请求")
        raw_export_item.addActionListener(ExportRawRequestAction(self, invocation))
        menu_items.append(raw_export_item)
    ...
    def extract_raw_requests(self, http_messages):
        """
        提取原始HTTP请求数据
        :param http_messages:
        :return:
        ...
        for message in http_messages:
            try:
                ...
                # TODO: 将字节数组转换为字符串，保持原始格式
                raw_request_str = self.bytes_to_utf8_string(request_bytes)
                
                raw_requests_data.append(raw_request_str)

            except Exception as e:
                self.log_error(u"提取原始请求数据错误", e)
                continue

        return raw_requests_data

    def save_raw_requests_to_file(self, raw_requests_data):
        """
        保存原始请求到txt文件
        :param raw_requests_data:
        :return:
        """
        chooser = JFileChooser()
        chooser.setDialogTitle(u"保存原始请求数据")

        # TODO: 生成默认文件名
        timestamp = self.get_current_time("timestamp")
        default_filename = "raw_requests_{}.txt".format(timestamp)
        chooser.setSelectedFile(File(default_filename))

        if chooser.showSaveDialog(None) == JFileChooser.APPROVE_OPTION:
            file_path = chooser.getSelectedFile().getPath()
            
            # TODO: 将多个请求合并，每个请求之间用两个空行分隔
            combined_content = "\n\n".join(raw_requests_data)
            
            # TODO: 确保文件末尾有适当的换行
            if not combined_content.endswith("\n"):
                combined_content += "\n"

            if self.write_utf8_file(file_path, combined_content):
                self._callbacks.printOutput(
                    u"原始请求数据保存成功: {}".format(file_path)
                )
                self._callbacks.printOutput(
                    u"共导出 {} 个原始请求".format(len(raw_requests_data))
                )
                return True
            else:
                return False
...
class ExportPythonAction(ActionListener):
    ...
    def actionPerformed(self, event):
        ...
        if not http_traffic:
            self.extender._callbacks.printError(u"未选择请求!")
            return
        ...
        try:
            ...
            self.extender._callbacks.printError(
                u"导出错误: {}".format(self.extender.safe_unicode(str(e)))
            )
...
class QuickExportAction(ActionListener):
    ...
    def actionPerformed(self, event):
        ...
        try:
            ...
            self.extender._callbacks.printError(
                u"快速导出错误: {}".format(self.extender.safe_unicode(str(e)))
            )
    ...
    def actionPerformed(self, event):
        ...
        try:
            ...
            if self.extender.save_api_data_to_file(api_data):
                self.extender._callbacks.printOutput(u"API接口导出完成!")

        except Exception as e:
            self.extender._callbacks.printError(
                u"API接口导出错误: {}".format(self.extender.safe_unicode(str(e)))
            )


# TODO: 导出原始请求处理
class ExportRawRequestAction(ActionListener):
    def __init__(self, extender, invocation):
        self.extender = extender
        self.invocation = invocation
    ...
    def actionPerformed(self, event):
        ...
        try:
            # TODO: 提取所有选中请求的原始请求数据
            raw_requests_data = self.extender.extract_raw_requests(http_traffic)

            if not raw_requests_data:
                self.extender._callbacks.printError(
                    u"没有可导出的原始请求数据!"
                )
                return
            ...
            # TODO: 保存原始请求数据到文件
            if self.extender.save_raw_requests_to_file(raw_requests_data):
                self.extender._callbacks.printOutput(u"原始请求导出完成!")

        except Exception as e:
            self.extender._callbacks.printError(
                u"原始请求导出错误: {}".format(self.extender.safe_unicode(str(e)))
            )
