...
class BurpExtender(IBurpExtender, IContextMenuFactory, IMessageEditorTabFactory):
    ...
    def load_export_path(self):
        ...
        try:
            ...
            if saved_path:
                ...
                if File(saved_path).exists():
                    self._callbacks.printOutput(
                        "已恢复导出路径: {}".format(saved_path).encode("utf-8")
                    )
                    return saved_path
                else:
                    self._callbacks.printOutput(
                        "之前配置的路径不存在，已清除: {}".format(saved_path).encode(
                            "utf-8"
                        )
                    )
                    # TODO: 清除无效的路径配置
            ...
            self._callbacks.printError(
                "加载配置错误: {}".format(self.safe_unicode(str(e))).encode("utf-8")
            )
            return None
    ...
    def save_export_path(self, path):
        ...
        try:
            if path:
                ...
                self._callbacks.printOutput(
                    "导出路径已保存: {}".format(path).encode("utf-8")
                )
            else:
                self._callbacks.saveExtensionSetting("export_path", None)
                self._callbacks.printOutput("导出路径配置已清除".encode("utf-8"))
        except Exception as e:
            self._callbacks.printError(
                "保存配置错误: {}".format(self.safe_unicode(str(e))).encode("utf-8")
            )
    ...
    def save_to_file(self, code):
        ...
        if chooser.showSaveDialog(None) == JFileChooser.APPROVE_OPTION:
            try:
                ...
                self._callbacks.printOutput("保存成功!".encode("utf-8"))
            except Exception as e:
                self._callbacks.printError(
                    "保存错误: {}".format(self.safe_unicode(str(e))).encode("utf-8")
                )
    ...
    def quick_save_to_file(self, code, method, url):
        ...
        if not self._export_path:
            self._callbacks.printError("未配置导出路径!".encode("utf-8"))
            return False
        ...
        try:
            ...
            self._callbacks.printOutput(
                "快速导出成功: {}".format(filename).encode("utf-8")
            )
            return True
        except Exception as e:
            self._callbacks.printError(
                "快速导出错误: {}".format(self.safe_unicode(str(e))).encode("utf-8")
            )
            return False
    ...
    def extract_api_data(self, http_messages):
        ...
        for message in http_messages:
            try:
                ...
                self._callbacks.printError(
                    "提取API数据错误: {}".format(self.safe_unicode(str(e))).encode(
                        "utf-8"
                    )
                )
                continue
    ...
    def save_api_data_to_file(self, api_data):
        ...
        """
        chooser = JFileChooser()
        chooser.setDialogTitle("保存API接口数据")
        ...
        if chooser.showSaveDialog(None) == JFileChooser.APPROVE_OPTION:
            try:
                ...
                self._callbacks.printOutput(
                    "API接口数据保存成功: {}".format(file_path).encode("utf-8")
                )
                self._callbacks.printOutput(
                    "共导出 {} 个API接口".format(len(api_data)).encode("utf-8")
                )
                return True
            except Exception as e:
                self._callbacks.printError(
                    "保存API数据错误: {}".format(self.safe_unicode(str(e))).encode(
                        "utf-8"
                    )
                )
                return False
...
class PythonTab(IMessageEditorTab):
    ...
    def safe_display_text(self, text):
        ...
        try:
            ...
            return "# 文本显示错误: {}".format(unicode(str(e)))
    ...
    def create_popup_menu(self):
        ...
        if self.extender._export_path:
            quick_save_item = JMenuItemCopy("快速保存")
        ...
        # TODO: 配置导出路径菜单项
        if self.extender._export_path:
            config_path_item = JMenuItemCopy("重新配置导出路径")
            ...
            current_path_item = JMenuItemCopy(
                "当前路径: {}".format(
                    self.extender._export_path[:30] + "..."
                    if len(self.extender._export_path) > 30
                    else self.extender._export_path
                )
            )
            ...
            # TODO: 清除路径配置菜单项
            clear_path_item = JMenuItemCopy("清除导出路径配置")
            clear_path_item.addActionListener(ClearPathTabAction(self.extender, self))
            popup_menu.add(clear_path_item)
        else:
            config_path_item = JMenuItemCopy("配置导出路径")
            config_path_item.addActionListener(ConfigPathTabAction(self.extender, self))
            popup_menu.add(config_path_item)

        # TODO: 刷新代码菜单项
        refresh_item = JMenuItemCopy("刷新代码")
        ...
        try:
            ...
            if headers and len(headers) > 0:
                request_line = headers[
                    0
                ]  # TODO: 第一行是请求行，如 "GET /path?query=value HTTP/1.1"
...
class ExportPythonAction(ActionListener):
    ...
    def actionPerformed(self, event):
        ...
        if not http_traffic:
            self.extender._callbacks.printError("未选择请求!".encode("utf-8"))
            return
        ...
        try:
            ...
            self.extender._callbacks.printError(
                "导出错误: {}".format(self.extender.safe_unicode(str(e))).encode(
                    "utf-8"
                )
            )
...
class QuickExportAction(ActionListener):
    ...
    def actionPerformed(self, event):
        ...
        try:
            ...
            self.extender._callbacks.printError(
                "快速导出错误: {}".format(self.extender.safe_unicode(str(e))).encode(
                    "utf-8"
                )
            )
...
class ExportAPIAction(ActionListener):
    ...
    def actionPerformed(self, event):
        ...
        try:
            ...
            if not api_data:
                self.extender._callbacks.printError(
                    "没有可导出的API数据!".encode("utf-8")
                )
                return

            # TODO: 保存API数据到文件
            if self.extender.save_api_data_to_file(api_data):
                self.extender._callbacks.printOutput("API接口导出完成!".encode("utf-8"))

        except Exception as e:
            self.extender._callbacks.printError(
                "API接口导出错误: {}".format(self.extender.safe_unicode(str(e))).encode(
                    "utf-8"
                )
            )
