...
def search_github(args):
    ...
    while len(results) < target:
        ...
        try:
            ...
            print(
                f"{COLOR['OKBLUE']}↳ 已获取 {len(results)}/{target} 剩余请求: {rate_limiter.remaining}{COLOR['ENDC']}",
                end='\r')
...
def export_to_md(results, filename):
    with open(filename, 'w', encoding='utf-8') as f:
        ...
        for idx, item in enumerate(results, 1):
            f.write(
                f"| {idx} | [{item['name']}]({item['url']}) | {item['description']} | ⭐ {item['stars']} | 🍴 {item['forks']} | {item['updated']} | [访问]({item['url']}) |\n")
